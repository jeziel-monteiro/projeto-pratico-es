
# 📝 Plano de trabalho 

| Campos | detalhes |
| :--- | :--- |
| **Nome do Projeto** | Porto Certo |
| **Versão** | 1.0 |
| **Status** | Iniciando projeto |
| **Número de Controle SRBR-M** | XXXXX |
| **Executor Principal** | Jeziel Monteiro Pessoa |
| **Coordernador do Projeto** | Andrey Antonio de Oliveira Rodrigues|

## 📜 Histórico de Versões

| Versão | Descrição | Autor | Data |
| :--- | :--- | :--- | :--- |
| 1.0 | ELaboração do trabalho | Jeziel Monteiro | 27/04/2026 |
| 2.0 | Especificação de requisitos | Sérgio Augusto | 05/05/2026

## Sumário
- [1 - 🌊 Introdução](#1---introdução)
  - [1.1 - 🎯 Objetivo](#11---objetivo)
  - [1.2 - 💡 Motivação, Justificativa, e Oportunidade](#12---motivação-justificativa-e-oportunidade)
  - [1.3 - 🏷️ Caraterização do Projeto](#13---caraterização-do-projeto)
    - [1.3.1 - Classe](#131---classe)
    - [1.3.2 - Enquadrabilidade](#132---enquadrabilidade)
- [2 - 🗺️ Informações Gerais](#2---informações-gerais)
  - [2.1 - Escopo Geral](#21---escopo-geral)
    - [2.1.1 - Escopo Específico](#211---escopo-específico)
    - [2.1.2 - Escopo negativo](#212---escopo-negativo)
  - [2.2 - 🛠️ Ambiente de Desenvolvimento](#22---ambiente-de-desenvolvimento)
  - [2.3 - ✨ Características inovadoras](#23---características-inovadoras)
  - [2.4 - 🏆 Resultados Esperados](#24---resultados-esperados)
- [3 - ⚙️ Metodologia](#3---metodologia)
  - [3.1 - 🏗️ Estrutura do Projeto](#31---estrutura-do-projeto)
  - [3.2 - 👨‍💻 Equipe de Projeto](#32---equipe-de-projeto)
- [4 - 💰 Despesas](#4---despesas)
  - [4.1 Dispêndios](#41-dispêndios)
    - [4.1.1 Equipamentos e Programa de Computador](#411-equipamentos-e-programa-de-computador)
    - [4.1.2 Materiais de Consumo](#412-materiais-de-consumo)
    - [4.1.3 Treinamento](#413-treinamento)
    - [4.1.4 Serviço Técnico de Terceiros](#414-serviço-técnico-de-terceiros)
  - [4.2 Resumo Financeiro e Valor Total](#42-resumo-financeiro-e-valor-total)

## 1 - 🌊 Introdução

<div align= "justify">
Em muitas regiões, o transporte fluvial tem um papel importante para a lógistica e locomoção, como o tráfego de cargas e viagem de pessoas, no entanto, tem-se uma necessidade em unificar essas informações. Por isso, o sistema é estruturada para automatizar o processo de compra para os usuários e centralizar a administração comercial para os operadores de transporte.
</div>

### 1.1 - 🎯 Objetivo

<div align= "justify">
O projeto consiste em desenvolver um sistema digital para a comercialização de passagens fluviais, possibilitando o cadastro de tripulantes e navegadores, gerenciamento de serviços de transporte e reserva de passagens online.
</div>

### 1.2 - 💡 Motivação, Justificativa, e Oportunidade

<div align= "justify">
A falta de informações para saber quanto é uma passagem de um barco, a data do embarque e desembarque, o local são algumas das dificuldades de muitos, tendo que procurar informação de terceiros. O nosso sistema visa esclarecer os dados de uma viagem fluvial em um ambiente só, sendo possível a reserva de passagens e o suporte de gerenciamento de operadores de transporte.
</div>

### 1.3 - 🏷️ Caraterização do Projeto

#### 1.3.1 - Classe

| Classe | Detalhamento |
| :--- | :--- |
| **Aplicativo Móvel** | Um app que conecta passageiros para compra e reserva de transportes fluviais |

#### 1.3.2 - Enquadrabilidade

| Enquadrabilidade | Detalhamento |
| :--- | :--- |
| Software Comercial | Um Software de compra de passagens para transportes fluviais | 

---

## 2 - 🗺️ Informações Gerais

### 2.1 - Escopo Geral

<div align= "justify">
O projeto visa desenvolver um aplicativo móvel para unificar informações de transporte fluviário, permitindo a compra de passagens online e gerenciamento de serviços de transporte.
</div>

#### 2.1.1 - Escopo Específico
- Cadastro e login do cliente e proprietário
- Sistema multi-perfil de acordo com a persona;
- Dashboard admnistrativo para o proprietário;
- Dashboard para compra e visualização para o cliente.

#### 2.1.2 - Escopo negativo
- Não será implementado funcionalidades para transporte de encomendas;
- Não serão implementadas funcionalidades avançadas de inteligência artificial.

### 2.2 - 🛠️ Ambiente de Desenvolvimento

| Componente | Tecnologia/ Ferramenta |
| :--- | :--- |
| Metodologia | Scrum com sprints |
| Gerenciamento de Backlog | Github|
| Repositório de Código | Github |
| Modelagem de Software | Github |
| Desenvolvimento MVP | XXXXX |

### 2.3 - ✨ Características inovadoras

<div align= "justify">
<ul>
  <li><b>Centralização de Informações Fluviais:</b> o sistema unifica dados cruciais sobre preços, datas, horários e locais de embarque e desembarque em uma única plataforma;</li>
  <li><b>Reserva e Compra Online de Passagens:</b> Permite a aquisição de passagens de transporte fluvial de forma totalmente digital e automatizada, eliminando a necessidade de deslocamento físico e filas;</li>
  <li><b>Gestão Integrada para Operadores:</b> Oferece um ambiente dedicado para que os operadores gerenciem seus serviços, viagens, embarcações e tripulação de forma eficiente e centralizada;</li>
  <li><b>Acessibilidade Móvel:</b> Sendo desenvolvido como um aplicativo móvel, garante que passageiros e operadores tenham acesso rápido e fácil a todas as funcionalidades do sistema.</li>
</ul>
</div>

### 2.4 - 🏆 Resultados Esperados

<div align= "justify">
<ul>
  <li><b>Facilidade de Acesso à Informação:</b> Redução significativa do tempo e esforço necessários para que os passageiros encontrem informações confiáveis e atualizadas sobre viagens fluviais;</li>
  <li><b>Aumento nas Vendas e Ocupação:</b> Com a conveniência da compra e reserva online, espera-se um aumento na taxa de ocupação das embarcações e na previsibilidade de receita para os operadores;</li>
  <li><b>Otimização da Gestão Comercial:</b> Melhoria no controle administrativo e operacional por parte dos operadores, reduzindo erros manuais e otimizando a alocação de recursos e tripulantes;</li>
  <li><b>Modernização do Setor Fluvial:</b> Inserção de tecnologia em um setor tradicionalmente carente de soluções digitais, promovendo maior segurança, transparência e previsibilidade para todos os envolvidos.</li>
</ul>
</div>

---

## 3 - ⚙️ Metodologia

### 3.1 - 🏗️ Estrutura do Projeto

<div style="text-align: justify;">
O desenvolvimento do projeto será dividido em cinco fases principais, seguindo a abordagem ágil com Scrum, utilizando sprints curtas para entregas iterativas. Fases do Projeto:
</div>

1. Ideação
2. Levantamento de Requisitos
3. Projeto e Arquitetura
4. Projeto dos Casos de Teste
5. Desenvolvimento do MVP

### 3.2 - 👨‍💻 Equipe de Projeto

| Papel | Responsabilidades |
| :--- | :--- |
| Arthur Djan | Designer de UI/UX |
| Heloyse Heloá | Software architect |
| Jeziel Monteiro | Product owner (PO) |
| Pâmilla Nunes | Software developer |
| Sérgio Augusto | Quality assurance (QA) engineer |

---

## 4 - 💰 Despesas

### 4.1 Dispêndios

#### 4.1.1 Equipamentos e Programa de Computador

<div align= "justify">
Para o desenvolvimento do sistema Porto Certo, são previstos os seguintes equipamentos e programas de computador:
</div>

| Item | Descrição | Quant. | Valor Unit. (R$) | Valor Total (R$) |
| :--- | :--- | :--- | :--- | :--- |
| 1 | Licença IDE de Desenvolvimento (anual) | 2 | 1.500,00 | 3.000,00 |
| 2 | Assinatura de Ferramenta de Design UI/UX (anual) | 1 | 1.200,00 | 1.200,00 |
| 3 | Licença de Banco de Dados (anual) | 1 | 2.000,00 | 2.000,00 |
| 4 | Ferramenta de Gerenciamento de Projetos (anual) | 1 | 800,00 | 800,00 |
| 5 | Computadores de Desenvolvimento (unidade) | 2 | 4.000,00 | 8.000,00 |
| 6 | Monitores Adicionais (unidade) | 2 | 700,00 | 1.400,00 |
| **Total** | | | | **16.400,00** |

#### 4.1.2 Materiais de Consumo

<div align= "justify">
Os materiais de consumo previstos para o desenvolvimento do projeto são os seguintes:
</div>

| Item | Descrição dos Equipamentos | Quant. | Valor unit (R$) | Valor Total (R$) |
| :--- | :--- | :--- | :--- | :--- |
| 1 | Materiais de escritório (resma de papel, canetas, etc.) | 1 | 300,00 | 300,00 |
| 2 | Cartuchos de tinta para impressora | 2 | 150,00 | 300,00 |
| 3 | Licenças de software de produtividade (mensal) | 3 | 50,00 | 150,00 |
| 4 | Assinatura de serviço de comunicação (mensal) | 1 | 100,00 | 100,00 |
| **Total** | | | | **850,00** |

#### 4.1.3 Treinamento

<div align= "justify">
Treinamentos não serão necessários ao projeto, pois as habilidades da equipe já cobrem as tecnologias a serem utilizadas, e eventuais aprendizados serão realizados via documentação e recursos online gratuitos.
</div>

#### 4.1.4 Serviço Técnico de Terceiros

<div align= "justify">
Os serviços técnicos de terceiros necessários para o desenvolvimento do projeto são:
</div>

| Item | Especificação | Custo (R$) |
| :--- | :--- | :--- |
| 1 | Consultoria especializada em UX/UI para aplicativos móveis | 5.000,00 |
| 2 | Serviço de hospedagem de servidor e banco de dados (anual) | 3.600,00 |
| 3 | Serviço de marketing digital e lançamento do aplicativo | 7.500,00 |
| 4 | Auditoria de segurança do código | 4.000,00 |
| **Total** | | **20.100,00** |

### 4.2 Resumo Financeiro e Valor Total

<div align= "justify">
Segue o resumo financeiro dos itens anteriores com os valores totais e porcentagem do aporte ao projeto:
</div>

| ITEM | VALOR TOTAL (R$) | % |
| :--- | :--- | :--- |
| Programas de Computador, Máquinas, Equipamentos, Aparelhos e Instrumentos | 16.400,00 | 16,40% |
| Implantação, Ampliação ou Modernização de Laboratório | 0,00 | 0,00% |
| Recursos Humanos Diretos | 45.000,00 | 45,00% |
| Recursos Humanos Indiretos | 0,00 | 0,00% |
| Livros e Periódicos Técnicos | 0,00 | 0,00% |
| Materiais de Consumo | 850,00 | 0,85% |
| Viagens | 2.500,00 | 2,50% |
| Treinamento | 0,00 | 0,00% |
| Serviço técnico de terceiros | 20.100,00 | 20,10% |
| Outros Correlatos | 5.000,00 | 5,00% |
| Provisão de Encerramento | 0,00 | 0,00% |
| Custos Incorridos e Constituição de Reserva | 0,00 | 0,00% |
| Margem 10% | 9.985,00 | 9,98% |
| Impostos Retidos em NF 19% | 18.971,50 | 18,97% |
| **TOTAL** | **113.806,50** | **100,00%** |